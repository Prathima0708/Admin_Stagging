...
import { ToastContainer, notifyError } from "./utils/toast";
...
const App = ({ deleteUserDetails }) => {
  useEffect(() => {
    document.title = process.env.REACT_APP_PRODUCT_ENV === "B2B" ? "BaddaMart" : "Baqaala";
  }, []);
  axios.interceptors.request.use((request) => {
    request.headers = {
      ...
      "x-timezone": timezone,
      "x-source": process.env.REACT_APP_PRODUCT_ENV === "B2B" ? "admin_b2b" : "admin_b2c",
    };
    ...
    if (request.params) {
      if (request.params["x-store-id"]) request.headers["x-store-id"] = Number(request.params["x-store-id"]);
    }
