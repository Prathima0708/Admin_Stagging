...
function BasicInfo(props) {
  ...
  const [shopLatLng, setShopLatLng] = useState(props.state?.shopLatLng || { lat: 25.286106, lng: 51.534817 });
  ...
  const [center, setCenter] = useState(props.state?.shopLatLng || { lat: 25.286106, lng: 51.534817 });
  const [onLoadOfComponent, setOnLoadOfComponent] = useState(true);
  const storeShowTimeCutOffOptions = [0, 1, 2, 3, 4, 5, 6, 7];
  const [storeShowTimeCutOff, setStoreShowTimeCutOff] = useState(storeShowTimeCutOffOptions[0]);
  ...
  // const [avgMonthlyPurchase, setAvgMonthlyPurchase] = useState("");
  // const [expPolicy, setExpPolicy] = useState("");
  // const [securityChequeFormData, setSecurityChequeFormData] = useState({});
  ...
  useEffect(() => {
    if (props.state !== null) {
      ...
      props.state.shopBuildingName && setShopBuildingName(props.state.shopBuildingName);
      ...
      props.state.storeShowTimeCutOff && setStoreShowTimeCutOff(props.state.storeShowTimeCutOff);
      props.state.shopImageFormData?.url && setShopImageFormData(props.state.shopImageFormData);
      props.state.storeEnabled && setStoreEnabled(props.state.storeEnabled);
      setStoreType(props.state.storeType);
      setCenter(props.state.shopLatLng);
      // setSecurityChequeFormData(props.state.securityChequeFormData);
      // setShopLandMark(props.state.address.landmark);
      // setCreditPeriod(props.state.creditPeriod);
      // setSecurity(props.state.security);
      // setSecurityAmt(props.state.securityAmt);
      // setAvgMonthlyPurchase(props.state.avgMonthlyPurchase);
      // setExpPolicy(props.state.expPolicy);
      // setNumberOfBranch(props.state.numberOfBranch);
      setLoading(false);
    }
  }, [props.state]);
  // useEffect(() => {
  //   if (shopLatLng) setCenter(shopLatLng);
  // }, [shopLatLng]);
  // gLatLng
  // useEffect(() => {
  //   if (gLatLng) setCenter(gLatLng);
  // }, [gLatLng]);
  // useEffect(async () => {
  //   if (shopLatLng === undefined) return;
  //   setCenter(shopLatLng);
  //   await fetch(
  //     `https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyDNgw8gwBZkqlYdvBxs00W2ROJFfmzT63s&latlng=${shopLatLng.lat},${shopLatLng.lng}`
  //   )
  //     .then((response) => response.json())
  //     .then((data) => {
  //       setShopLocality(undefined);
  //       setShopStreet(undefined);
  //       setShopCity(undefined);
  //       setLandmark(undefined);
  //       const locality = data.results.filter((d) => d.types?.includes("sublocality"))[0]
  //         ?.address_components[0]?.long_name;
  //       locality !== undefined ? setShopLocality(locality) : setShopLocality("");
  //       const street = data.results.filter((d) => d.types?.includes("route"))[0]?.address_components[0]
  //         ?.long_name;
  //       street !== undefined ? setShopStreet(street) : setShopStreet("");
  //       const city = data.results.filter((d) => d.types?.includes("locality"))[0]?.address_components[0]
  //         ?.long_name;
  //       city !== undefined ? setShopCity(city) : setShopCity("");
  //       const postCode = data.results.filter((d) => d.types?.includes("postal_code"))[0]
  //         ?.address_components[0]?.long_name;
  //       postCode !== undefined ? setLandmark(postCode) : setLandmark("");
  //     });
  // }, [shopLatLng]);

  const returnWorkingHoursArray = () => {
    const openDays = workingHours.filter((day) => day.storeOpen);
    const definedTimes = openDays.filter((day) => day.openTime && day.closeTime);
    ...
    definedTimes.forEach((day) => {
      ...
      const closeTimeInMinutes = Number(closeTimeArray[0]) * 60 + Number(closeTimeArray[1]);
      const workingTimeInMins = closeTimeInMinutes - openTimeInMinutes;
      workingHoursArray.push({ dayOfWeek: day.id, openTime: open?.time, workingTimeInMins });
    });
  ...
  const renderDayWorkingHourInput = () => {
    return workingHours.map(
      ({ id, day, storeOpen, openTimeOption, closeTimeOption, openTime, closeTime }) => (
        <div key={id} className="flex flex-col md:flex-row place-content-between ">
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      setWorkingHours((prevData) => {
                        ...
                        prevData[id].closeTimeOption = stopTimeOption.slice(Number(e.target.value));
                        return [...prevData];
                      })
                    }
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      setWorkingHours((prevData) => {
                        ...
                        prevData[id].openTimeOption = startTimeOption.slice(0, Number(e.target.value) + 1);
                        return [...prevData];
                      })
                    }
                    className="border h-12 text-sm focus:outline-none block w-full bg-gray-100 dark:bg-white border-transparent focus:bg-white"
          ...
          <Grid container spacing={3}>
            ...
            <Grid item lg={6} md={12} sm={12} xs={12} className="">
              <Label>
                ...
                <Map zoom={14} center={center} shopLatLng={shopLatLng} setShopLatLng={setShopLatLng}></Map>
              </Label>
            </Grid>
            ...
            <Grid item lg={6} md={6} sm={12} xs={12} className=" dark:text-gray-300 text-gray-900">
            ...
            <Grid item lg={6} md={12} sm={12} xs={12} className="">
              ...
              {shopImageFormData.url ? (
                <ExistingImageDisplay setFormData={setShopImageFormData} url={shopImageFormData.url} />
              ) : (
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  let businessInfo = {
                    ...
                    storeType: process.env.REACT_APP_PRODUCT_ENV === "B2B" ? storeType : "SELLER",
                  ...
                  let state = {
                    ...
                    shopPhone,
                    shopEmail,
                    shopLatLng: shopLatLng ? shopLatLng : { lat: 25.286106, lng: 51.534817 },
