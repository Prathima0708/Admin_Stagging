...
const Filter = ({ active, brand, category }) => {
  ...
  const renderBrandCheckboxFilter = () => {
    return (
      <div className="border-green-500 border-2 rounded-md flex flex-col  my-2 ">
        ...
        >
          <div className="">
            <span className="text-gray-700 dark:text-gray-300">Filter By Brand</span>
          </div>
          <Checkbox checked={brandExpanded} color="info" style={{ color: "green" }} />
        </div>
        {brandExpanded && (
          <>
            <Input
            ...
            {brandList.length > 0 &&
              brandList
                .filter(
                  ({ name }) => name.toLowerCase().includes(brandFilterText) || brandFilterText.length === 0
                )
                .map(({ id, name }) => (
                  <div
                    key={id}
    ...
    return (
      <div className="border-green-500 border-2 rounded-md flex flex-col  my-2">
        ...
        >
          ...
          <Checkbox checked={categoryExpanded} color="info" style={{ color: "green" }} />
        </div>
        {categoryExpanded && (
          <>
            <Input
            ...
            {categoryList.length > 0 &&
              categoryList
                .filter(
                  ({ name }) =>
                    name.toLowerCase().includes(categoryFilterText) || categoryFilterText.length === 0
                )
                .map(({ id, name }) => (
                  <div
                    key={id}
  ...
  return (
    <div className=" w-2/5 border-gray-400 rounded-md mr-4 border-2 p-2 " style={{ minHeight: "500px" }}>
      <Scrollbars>
